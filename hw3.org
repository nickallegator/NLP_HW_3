...
Examples of conflicting suffixes and their tags:
  Suffix: mer, Tags: {'ADJ', 'NOUN'}
  Suffix: ied, Tags: {'VERB', 'ADJ'}
  Suffix: her, Tags: {'CONJ', 'ADP', 'DET', 'NOUN'}
  Suffix: ust, Tags: {'VERB', 'NOUN'}
  Suffix: tic, Tags: {'ADJ', 'NOUN'}
Dataset: Test
Total unique suffixes: 82
Conflicting suffixes: 1
Examples of conflicting suffixes and their tags:
  Suffix: ing, Tags: {'VERB', 'NOUN'}
...
4. [@4] <<lda>> Continuing with the Bayesian theme of problem [[bayesian]],
 ...
 #+begin_example
 LDA with and without downcasing yields identical test accuracy (0.68) and the same number of
 features (370). This suggests that case information is not adding useful signal to these three
 letter suffix features. This is likely because most Brown news tokens are already lowercase
 and suffixes are mostly unaffected by capitalization. The classifier heavily favors NOUN and X,
 with very low recall for ADJ, ADP, and VERB, indicating that suffix alone is a weak predictor
 for many POS tags.
 #+end_example
...
5. [@5] <<bow>> One feature that might be helpful in distinguishing
   ...
   complete and self-contained.
	   ...
	   def main():
	       ...
	       # All indices already used by first anchors and their successors
	       first_used = set(first_anchors) | set(first_succ)

	       # ----- Second anchors/control -----
	       candidate_second_anchors = [
	           j for j in valid_anchor_indices
	           if (j not in first_used) and ((j + 1) not in first_used)
	       ]
...
6. [@6] <<ml>> Choose 500 random sentences from the Brown news corpus
   ...
   #+begin_src python :results output
     ...
     def extract_contexts():
         ...
         # Make it indexable
         tagged = list(brown.tagged_sents(categories="news", tagset="universal"))

         # Precompute which sentence indices are eligible
         candidate_indices = []
         for idx, s in enumerate(tagged):
             tags = [t for _, t in s]
             noun_pos = [i for i, t in enumerate(tags) if t == "NOUN"]
             if any(i >= 3 for i in noun_pos):
                 candidate_indices.append(idx)

         if len(candidate_indices) < NUM_SENTENCES:
             raise ValueError(
                 f"Not enough sentences with a noun at position ≥ 3 "
                 f"({len(candidate_indices)} found, need {NUM_SENTENCES})."
             )

         # Sample sentences WITHOUT replacement
         selected_indices = random.sample(candidate_indices, NUM_SENTENCES)
         selected_indices = sorted(selected_indices)
         selected = [tagged[i] for i in selected_indices]

         # Build contexts: three tags immediately before each noun at index ≥ 3
         contexts = []
         for s in selected:
             tags = [t for _, t in s]
   ...
   #+end_src
   #+RESULTS:
   : Saved 3160 encoded chunks to q6_contexts.json
   ...
   #+begin_src python :results output
     ...
     def run_meanshift():
         # Explicitly estimate a bandwidth instead of relying on defaults
         ...
         plt.title(f"MeanShift Clusters (bandwidth={bandwidth:.3f})")
     ...
     def run_agg(linkage, metric=None, k_values=range(2, 7)):

         # Try k from 2-6
         print(f"\nAgglomerativeClustering: linkage={linkage}, metric={metric}")
         ...
         for k in k_values:
             if linkage == "ward":
                 # Ward requires Euclidean and does not take metric argument
             ...
             # If it collapses to 1 cluster, silhouette is undefined; skip.
             if len(set(labels)) < 2:
                 print(f"  k={k}: only one cluster, skipping silhouette.")
                 continue
         ...
         print(f"Best k for linkage={linkage}, metric={metric}: {best_k} (silhouette={best_score:.4f})")

         # Plot using the best k
         plt.figure()
         title_metric = metric_label if linkage == "ward" else metric
         plt.scatter(X2[:, 0], X2[:, 1], c=best_labels, s=10)
         plt.title(f"{linkage} linkage ({title_metric}), k={best_k}")
         fname_metric = "euclidean" if linkage == "ward" else metric
         plt.savefig(f"agg_{linkage}_{fname_metric}_k{best_k}.png", dpi=220)
         plt.close()

         print(f"Saved plot: agg_{linkage}_{fname_metric}_k{best_k}.png")
   ...
   #+RESULTS:
   #+begin_example
   MeanShift bandwidth: 5.436697644204795
   MeanShift clusters: 1

   AgglomerativeClustering: linkage=ward, metric=None
     k=2: silhouette=0.2256
     k=3: silhouette=0.2602
     k=4: silhouette=0.3158
     k=5: silhouette=0.3293
     k=6: silhouette=0.3445
   Best k for linkage=ward, metric=None: 6 (silhouette=0.3445)
   Saved plot: agg_ward_euclidean_k6.png

   AgglomerativeClustering: linkage=average, metric=manhattan
     k=2: silhouette=0.2446
     k=3: silhouette=0.2449
     k=4: silhouette=0.3090
     k=5: silhouette=0.2866
     k=6: silhouette=0.3133
   Best k for linkage=average, metric=manhattan: 6 (silhouette=0.3133)
   Saved plot: agg_average_manhattan_k6.png

   AgglomerativeClustering: linkage=average, metric=cosine
     k=2: silhouette=0.2878
     k=3: silhouette=0.2608
     k=4: silhouette=0.2509
     k=5: silhouette=0.2198
     k=6: silhouette=0.2776
   Best k for linkage=average, metric=cosine: 2 (silhouette=0.2878)
   Saved plot: agg_average_cosine_k2.png
   #+end_example

#+caption: MeanShift clustering of Brown tag-context encodings
#+name: q6-meanshift
#+attr_latex: :placement [!t]
[[./meanshift.png]]

#+caption: Agglomerative clustering with ward linkage (Euclidean)
#+name: q6-agg-ward
#+attr_latex: :placement [!t]
[[./agg_ward_euclidean_k6.png]]

#+caption: Agglomerative clustering with average linkage and Manhattan metric
#+name: q6-agg-average-manhattan
#+attr_latex: :placement [!t]
[[./agg_average_manhattan_k6.png]]

#+caption: Agglomerative clustering with average linkage and cosine metric
#+name: q6-agg-average-cosine
#+attr_latex: :placement [!t]
[[./agg_average_cosine_k2.png]]

 #+begin_example
 We took 3,178 three tag contexts before nouns from 500 Brown “news” sentences and mapped them
 into a 3D tag space. After which we projected them into 2D space with PCA to visualize them
 easier. When we ran MeanShift with the default settings we only produced a single cluster.
 However, the different versions of AgglomerativeClustering each showed two groups in the space.
 AgglomerativeClustering with ward linkage and Euclidean distance breaks the points into two
 intertwined clouds where one shows up more towards the bottom right and one more towards the
 top left. AgglomerativeClustering with average linkage and Manhattan distance creates two far
 less intertwined groups divided horizontally with a large bias towards the bottom group.
 AgglomerativeClustering with average linkage and cosine distance produces clouds that resemble
 the distribution of AgglomerativeClustering with ward linkage and Euclidean distance, with one
 group showing up more towards the top left and the other the bottom right, but has a far cleaner
 separation between the two groups. This shows that there is some weak structure in these tag
 context prevalent enough for the AgglomeritiveClustering to find two cluster solutions, however
 the exact boundary is determined by its respective distance. Additionally, these findings show
 that MeanShift’s behavior with only three POS tags per point has density that is too smooth to
 find multiple clusters without tweaking the bandwidth.
 #+end_example
