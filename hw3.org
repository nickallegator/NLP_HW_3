...
Several three-letter suffixes appear in words belonging to different POS categories.
Since the classifier cannot reliably learn a one-to-one mapping in those cases,
these suffixes are reassigned to the label `X`. This prevents the model from
memorizing inconsistent or ambiguous suffix-to-tag pairs, and helps reduce
confusion that would negatively impact classification accuracy.
...
The Linear Discriminant Analysis classifier only sees a single feature per token:
the three-letter suffix string. This creates a **very small and highly overlapping
feature space**, where different POS categories share many of the same suffixes.
As a result, the model struggles to separate classes cleanly.

Downcasing the suffixes did not reduce feature dimensionality or improve accuracy.
This confirms that **case carries almost no useful information** in this simplified
representation. More informative features (e.g., full morphological patterns or
surrounding context) would be needed for LDA to produce stronger results.
...
The baseline comparison (first anchors and their real successors) shows the highest
average overlap — because consecutive sentences in news articles are usually about
the same topic, so they naturally reuse keywords.

The second random-anchor set has similar overlap statistics, confirming that the
effect is not simply caused by which anchor sentences were selected.

However, once the corpus is **permuted**, the overlap drops sharply. The mean
intersection size falls from ~0.65 to ~0.05, and almost **95% of sentence pairs
share zero significant words**.

This shows that even a weak amount of lexical repetition contributes to coherence.
Neighboring sentences in real text tend to remain topically related, while shuffled
sentences lack any structural continuity. Although this signal alone is not strong
enough for full classification, it still reflects an underlying difference between
human-written order and random ordering.
    ...
    for chunk in contexts:
        ...
        for t in chunk:
            if t not in tag_map:
                tag_map[t] = idx
                idx += 1
            tup.append(tag_map[t])
        mapped.append(tup)

    with open("q6_contexts.json", "w") as f:
        json.dump(mapped, f)

    print("Saved", len(mapped), "contexts.")

if __name__ == "__main__":
    extract_contexts()
#+end_src

** Clustering Code + Plots
#+begin_src python :results output
import json
import numpy as np
...
def run_agg(linkage, metric=None):
    if linkage == "ward":
        ...
        fname = "agg_ward_None.png"
    else:
        agg = AgglomerativeClustering(linkage=linkage, metric=metric)
        fname = f"agg_{linkage}_{metric}.png"

    labels = agg.fit_predict(X)
    plt.figure()
    plt.scatter(X2[:,0], X2[:,1], c=labels, s=10)
    plt.title(f"{linkage} linkage ({metric})")
    plt.savefig(fname, dpi=220)
    plt.close()
    print(fname, "clusters:", len(set(labels)))
...
print("Saved all cluster plot images!")
#+end_src
...
[[file:meanshift.png]]
#+ATTR_LATEX: :width 0.85\linewidth
[[file:agg_ward_None.png]]
#+ATTR_LATEX: :width 0.85\linewidth
[[file:agg_average_manhattan.png]]
#+ATTR_LATEX: :width 0.85\linewidth
[[file:agg_average_cosine.png]]

** Discussion
All methods used the same encoded 3-tag context features, but the geometry of the clusters differs:

| Method | Behavior | Interpretation |
|--------+----------+----------------|
| MeanShift | produced many tiny clusters | algorithm is sensitive to noise and local density → probably over-segmenting |
| Ward (Euclidean) | gives the **most compact** groups | minimizing variance works best for this discrete feature space |
| Average + Manhattan | medium-spread clusters | less strict grouping, good when absolute differences matter |
| Average + Cosine | very spread out clusters | groups tag *patterns* by direction, not by exact values |

Overall:  
The contexts before nouns contain **weak but real** structure.  
Ward clustering gives the clearest separation, implying slight syntactic regularities.  
The small dimensionality (only 3 tags as integers) limits how strongly grammar shows up in clusters.

So, the preceding tags influence nouns in a *loosely consistent* way, but more features would be needed to reveal true linguistic categories.
